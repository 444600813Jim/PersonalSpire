...
internal class Class173 : Class172
{
    ...
    internal Class173(int A_0, int A_1) : base(A_0, A_1)
    {
    }

    internal Class173(int A_0, Class456[] A_1) : this(A_0, A_1.Length, 8)
    {
    ...
    internal Class173(int A_0, Class679[] A_1) : this(A_0, A_1.Length, 2)
    {
        using (MemoryStream stream = new MemoryStream(this.byte_0))
        {
            BinaryWriter writer = new BinaryWriter(stream);
            for (int i = 0; i < A_1.Length; i++)
            {
                int num2 = Class600.smethod_4(A_1[i]);
                writer.Write((ushort) num2);
            }
        }
    }

    internal Class173(int A_0, Class70[] A_1) : this(A_0, A_1.Length, 0x10)
    {
        using (MemoryStream stream = new MemoryStream(this.byte_0))
        {
            BinaryWriter writer = new BinaryWriter(stream);
    ...
    internal Class173(int A_0, Class782[] A_1) : this(A_0, A_1.Length, 12)
    {
        using (MemoryStream stream = new MemoryStream(this.byte_0))
        {
            BinaryWriter writer = new BinaryWriter(stream);
    ...
    internal Class173(int A_0, Class784[] A_1) : this(A_0, A_1.Length, 0x24)
    {
        using (MemoryStream stream = new MemoryStream(this.byte_0))
        {
            BinaryWriter writer = new BinaryWriter(stream);
    ...
    internal Class173(int A_0, Class891[] A_1) : this(A_0, A_1.Length, 8)
    {
        using (MemoryStream stream = new MemoryStream(this.byte_0))
        {
            BinaryWriter writer = new BinaryWriter(stream);
    ...
    internal Class173(int A_0, Class954[] A_1) : this(A_0, A_1.Length, 8)
    {
        using (MemoryStream stream = new MemoryStream(this.byte_0))
        {
            BinaryWriter writer = new BinaryWriter(stream);
    ...
    internal Class173(int A_0, int[] A_1) : this(A_0, A_1.Length, 4)
    {
        using (MemoryStream stream = new MemoryStream(this.byte_0))
        {
            BinaryWriter writer = new BinaryWriter(stream);
    ...
    private Class173(int A_0, int A_1, int A_2) : base(A_0, smethod_0(A_1, A_2))
    {
        this.int_3 = A_1;
        this.int_4 = A_1;
        this.int_5 = A_2;
    ...
    internal Class891[] method_5()
    {
        ...
        using (MemoryStream stream = new MemoryStream(this.byte_0))
        {
            ...
            if (num2 != 4)
            {
                if (num2 != 8)
                {
                    throw new InvalidOperationException(BookmarkStart.b("支就弳堵圷䴹刻ḽ☿ⵁ㙃⭅⥇㹉汋⡍㽏⁑瑓╕し㭙ⱛ㭝䁟ቡգብg䑩", num));
                }
                for (int i = 0; i < classArray.Length; i++)
                {
    ...
    internal Class679[] method_9()
    {
        ...
        using (MemoryStream stream = new MemoryStream(this.byte_0))
        {
            ...
            for (int i = 0; i < classArray.Length; i++)
            {
                ...
                classArray[i] = Class600.smethod_3(num2);
            }
            return classArray;
        }
    }
