...
internal class Class1155
{
    ...
    [CLSCompliant(false)]
    public static uint smethod_0(byte[] A_0, int A_1, int A_2, uint A_3)
    {
        int num = 0;
        if (A_0 == null)
        {
        ...
        while (index < num4)
        {
            uint num3 = uint_0[(int) ((IntPtr) ((A_3 ^ A_0[index]) & 0xff))];
            A_3 = (A_3 >> 8) ^ num3;
            index++;
        }
        return ~A_3;
    ...
    [CLSCompliant(false)]
    public static uint smethod_1(Stream A_0, int A_1)
    {
        int num = 0x10;
        if (A_0 == null)
        {
        ...
        if ((num2 < A_1) || (A_1 < 0))
        {
            throw new ArgumentOutOfRangeException(BookmarkStart.b("娵崷吹嬻䨽⠿", num));
        }
        byte[] buffer = new byte[Math.Min(num2, 0x1000)];
        while (num5 > 0)
        {
            int count = Math.Min(num5, 0x1000);
            int num6 = A_0.Read(buffer, 0, count);
            if (num6 == 0)
            {
                throw new Exception2(BookmarkStart.b("猵嘷帹᰻儽☿扁㝃㉅㩇⽉ⵋ⍍灏⁑ㅓ㝕㭗㉙㥛㩝", num));
            }
            num4 = smethod_0(buffer, 0, num6, num4);
            num5 -= num6;
        }
        return num4;
    }
