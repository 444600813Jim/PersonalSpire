...
internal sealed class Class299
{
    ...
    internal int method_1(Stream A_0, byte[] A_1)
    {
        int num = 0x13;
        if (A_0.Read(A_1, 0, 4) != 4)
        {
            throw new Exception(BookmarkStart.b("瀸唺䬼帾ⵀ⩂⅄杆ⵈ⩊㥌⹎", num));
        }
        return BitConverter.ToInt32(A_1, 0);
    }

    internal byte[] method_10(HashAlgorithm A_0, string A_1, byte[] A_2, byte[] A_3, int A_4, int A_5)
    {
        byte[] bytes = Encoding.Unicode.GetBytes(A_1);
        byte[] buffer2 = A_0.ComputeHash(this.method_12(A_2, bytes));
        for (int i = 0; i < A_5; i++)
        {
            buffer2 = A_0.ComputeHash(this.method_12(BitConverter.GetBytes(i), buffer2));
        }
        buffer2 = A_0.ComputeHash(this.method_12(buffer2, A_3));
        return this.method_13(buffer2, A_4, 0x36);
    }
    ...
    internal byte[] method_13(byte[] A_0, int A_1, byte A_2)
    {
    ...
    internal byte[] method_14(byte[] A_0, byte[] A_1)
    {
    ...
    internal bool method_15(byte[] A_0, byte[] A_1)
    {
    ...
    internal string method_2(Stream A_0)
    {
        int num = 5;
        byte[] buffer = new byte[4];
        int count = this.method_1(A_0, buffer);
        buffer = new byte[count];
        if (A_0.Read(buffer, 0, count) != count)
        {
            throw new Exception(BookmarkStart.b("截䌬央倰弲尴匶ᤸ强尼䬾⁀", num));
        }
    ...
    internal string method_3(Stream A_0)
    {
    ...
    internal void method_4(Stream A_0, int A_1)
    {
    ...
    internal void method_6(Stream A_0, string A_1)
    {
    ...
    internal byte[] method_7(string A_0, byte[] A_1, int A_2)
    {
        byte[] buffer8;
    ...
    internal byte[] method_8(string A_0, byte[] A_1, byte[] A_2, int A_3, int A_4, bool A_5)
    {
        ...
        byte[] bytes = Encoding.Unicode.GetBytes(A_0);
        byte[] buffer = algorithm.ComputeHash(this.method_12(A_1, bytes));
        for (int i = 0; i < A_4; i++)
        {
            buffer = algorithm.ComputeHash(this.method_12(BitConverter.GetBytes(i), buffer));
        }
        buffer = algorithm.ComputeHash(this.method_12(buffer, A_2));
        return this.method_13(buffer, A_3, 0x36);
    }

    internal byte[] method_9(string A_0, byte[] A_1, byte[] A_2, int A_3, int A_4)
    {
        return this.method_8(A_0, A_1, A_2, A_3, A_4, false);
    }

    internal delegate void Delegate2(byte[] A_0, byte[] A_1);
